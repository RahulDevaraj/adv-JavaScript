...
result = jobs.filter(job=>job.jobStatus==="completed").reduce((prev,cur)=> `${prev.name} and ${cur.name}`)
jobs.filter(job=>job.jobStatus==="completed").reduce((prev,cur)=> `${prev.name} and ${cur.name}`)
console.log(result)